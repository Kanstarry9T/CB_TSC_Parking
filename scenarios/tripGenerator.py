...
'''
Develop a traffic generator to mimic day-to-day traffic demand variations
- background traffic
- special event trips
    --critical path/route traffic
'''
...
trip_fp = os.path.join(scen_folder, 'routes0.rou.xml') # the initial trips generated by randomTrip.py
park_trip_fp = os.path.join(scen_folder, 'routes.rou.xml')
park_fp = os.path.join(scen_folder, 'parkings.add.xml')

class Trip:
    def __init__(self, idx, depart, edges):
        self.id = idx
        self.depart = depart
        self.links = edges # road links to travel on
        self.stop = None
        self.duration = None
    
    def contain(self, link):
        if link in self.links:
            return True
        else:
            return False
   
    def set_stop(self, stop_link, stop_duration):
        self.stop = 'PA_' + stop_link # parkingArea to stop
        self.duration = stop_duration


def process_trips():
    tree = ET.parse(trip_fp)
    root = tree.getroot()

    trips = list()
    
    for child in root:
        idx, depart = child.attrib['id'], child.attrib['depart']
        # print(idx)
        edges = child[0].attrib['edges'].split()
        trip = Trip(idx, depart, edges)

        trips.append(trip)
    
    return trips

def process_parkings(trips, max_occ=100):
    tree = ET.parse(park_fp)
    root = tree.getroot()

    park_links = dict()

    for child in root:
        park_id, park_lane, cap = child.attrib['id'], child.attrib['lane'], child.attrib['capacity']
        park_link = park_lane.split('_')[0]
        park_links[park_link] = [park_id, cap]
    

    park_trips = {k: [] for k in park_links.keys()}
    park_durations = {k: [] for k in park_links.keys()}
    parkInfo = dict()

    for link in park_links.keys():

        idx = park_links[link][0]
        cap = int(park_links[link][1])
        numtrips = 0
        for trip in trips:
            if trip.stop == None and trip.contain(link):
                duration = select_duration()
                trip.set_stop(link, duration)
                park_trips[link].append(trip)
                numtrips += 1
                park_durations[link].append(duration)
            else:
                continue
        
            # post process for each parking link
            dur = round(np.mean(park_durations[link]), 1)
            occ = round(numtrips / cap, 3)
            tmp = {'id': idx, 'cap': cap, 'occ': occ, 'avg_dur': dur}        
            if occ >= max_occ:
                break

        parkInfo[link] = tmp
            
    return trips, park_trips, parkInfo
    

def select_duration(min_dur=600, max_dur=1200):
    return np.random.randint(min_dur, max_dur) 

def trips2xml(trips, fp=park_trip_fp):
    '''
    write trips to a sumo routes.rou.xml file
    '''
    dom = minidom.getDOMImplementation().createDocument(None,'routes',None)
    root = dom.documentElement

    for trip in trips:
        veh = dom.createElement('vehicle')
        veh.setAttribute('id', trip.id)
        veh.setAttribute('depart', trip.depart)

        route = dom.createElement('route')
        str_edges = " ".join(trip.links)
        route.setAttribute('edges', str_edges)

        veh.appendChild(route)

        if trip.stop:
            stop = dom.createElement('stop')
            stop.setAttribute('parkingArea', trip.stop)
            stop.setAttribute('duration', str(trip.duration))
...
def main():
    trips = process_trips()
    trips, park_trips, parkInfo = process_parkings(trips)
    trips2xml(trips)
    
    # double-check the final results
    for k, v in parkInfo.items():
        print('the park info is {}'.format(v))
        print('the number of trips is {}'.format(len(park_trips[k])))
...
class DemandGenerator:
    '''
    Generate pairwise OD zones traffic demand
    '''
    def __init__(self, trafficZones):
        pass
...
class TrafficGenerator:
    '''
    Develop a zone-based traffic generator, the following laws/models may be used
    - zone-level total trip demand follows scaling law 
    - pairwise origin-destination pairwise zones follows the gravity law (or others)
    - trip distance (time?) distribution follows zipf's law
    - route choice follows BPR model

    Four-step model (Top-down method) for traffic generation: https://github.com/asu-trans-ai-lab/grid2demand
    - generate traffic zones
    - estimate trip production and trip attraction of each zone (scaling law)
    - estimate pairwise OD-zones demand (gravity law / zipf's law / visitation law / others)
    - origin/destination choice and route choice (BPR model)
    '''
    ...
    def run(self):
        ...
        t1 = time.time()
        print("Traffic generation time is {}".format(t1-t0)) 
            
    def write_data(self):    
        '''write data to files'''
        data_path = os.path.join(self.output_folder, 'trips.xml')
        self._toSUMO(output_path=data_path)
...
class Flow:
    ...
    def get_trafficZones(self, numRows=6, numColumns=8, Oprob_mode=3, prob_corner=0.2, prob_mid=0.15):
        '''

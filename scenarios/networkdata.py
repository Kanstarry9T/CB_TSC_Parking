...
class NetworkData:
    ...
    def __init__(self, net_fp):
        print(net_fp) 
        self.net = sumolib.net.readNet(net_fp) # https://sumo.dlr.de/daily/pydoc/sumolib.net.html
        ###get edge data
    ...
    def get_net_data(self):
        return {
            'lane':self.lane_data, 
            'edge':self.edge_data, 
            # 'origin':self.find_origin_edges(), 
            # 'destination':self.find_destination_edges(), 
            'node':self.node_data, 
    ...
    def get_edge_data(self, net):
        ...
        for edge in edges:
            ...
            edge_data[edge_ID]['coord'] = ' '.join([str(incnode_coord[0]), str(incnode_coord[1]), str(outnode_coord[0]), str(outnode_coord[1])])
            # np.array([incnode_coord[0], incnode_coord[1], outnode_coord[0], outnode_coord[1]]).reshape(2,2)
    ...
    def get_node_data(self, net):
        ...
        for node in nodes:
            ...
            node_data[node_id]['incoming'] = [str(edge.getID()) for edge in node.getIncoming()]
            node_data[node_id]['outgoing'] = [str(edge.getID()) for edge in node.getOutgoing()]

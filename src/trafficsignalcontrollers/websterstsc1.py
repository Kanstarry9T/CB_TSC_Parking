...
class WebstersTSC(TrafficSignalController):
    ...
    def initialize(self):
        ...
        self.split_NEMAphase(self.leadLag_mode, self.nyrs) # step 1.2
    ...
    def update(self, data):
        ...
        if self.phase_switch == self.nPhases[-1]:
            self.cycle_cnt += 1
            self.phase_switch = 0
            if self.cycle_cnt % self.update_freq == 0: # update the signal timing
    ...
    def split_NEMAphase(self, leadLag_mode, nyrs):
        if nyrs == 4:
            default_splits = {
    ...
    def get_phaseCycle(self, previous=False):
        '''
        get a standard "phase-cycle" data structure, more friendly for setting SUMO signal tming
        '''

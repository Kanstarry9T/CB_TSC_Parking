...
def parse_cl_args():
    ...
    #multi proc params
    parser.add_argument("-n", type=int, default=1, dest='n', help='number of sim procs (parallel simulations) generating experiences, default: os.cpu_count()-1')
    parser.add_argument("-l", type=int, default=1, dest='l', help='number of parallel learner procs producing updates, default: 1')

    ##sumo params
    parser.add_argument("-sim", type=str, default='grid', dest='sim', help='simulation scenario, default: lust, options:lust, single, double')
    ...
    parser.add_argument("-res_fp", type=str, default=None, dest='res_fp', help='path to save output into csv file')
    ...
    parser.add_argument("-mode", type=str, default='test', dest='mode', help='reinforcement mode, train (agents receive updates) or test (no updates), default:train, options: train, test'  )
    ...
    parser.add_argument("-simlen", type=int, default=2400, dest='sim_len', help='length of simulation in seconds/steps')
    parser.add_argument("-gui", default=False, action='store_true', dest='gui', help='enable gui, default: False')
    ...
    #shared tsc params
    parser.add_argument("-gmin", type=int, default=10, dest='g_min', help='minimum green phase time (s), default: 5')
    parser.add_argument("-y", type=int, default=3, dest='y', help='yellow change phase time (s), default: 2')
    parser.add_argument("-r", type=int, default=1, dest='r', help='all red stop phase time (s), default: 3')
    parser.add_argument("-p", type=float, default=.0, dest='p', help='penetration rate of online adaptive traffic signal controllers, default: .0')

    # parser.add_argument("-tscfp", type=str, default='output/grid0/tsc.json', dest='tsc_fp', help='path to saved signal timing data')
    # parser.add_argument("-out", type=str, default='output/grid0/', dest='out', help='path to saved signal timing data')

 
    #pretimed params
    parser.add_argument("-save_tsc", default=None, dest='save_tsc', help='save traffic signal timing into a predefined path, default: None')
    # parser.add_argument("-tscfp", type=str, dest='tscfp', help='recorded signal timing file')

    parser.add_argument("-load_tsc", default=None, dest='load_tsc', help='load traffic signal timing from the defined path, default: None')
    ...
    parser.add_argument("-satflow", type=float, default=0.5, dest='sat_flow', help='lane vehicle saturation rate (veh/s), default: 0.38')
    parser.add_argument("-f", type=int, default=1, dest='update_freq', help='interval over which websters timing are computed (s), default: 900')
    ...
    parser.add_argument("-gmax", type=int, default=60, dest='g_max', help='maximum green phase time (s), default: 120')

    # if arg_dict:
    #     print("update arguments")
    #     for k, v in arg_dict.items():
    #         remove_options(parser, [k])
    #         parser.add_argument(k, type=v['type'], default=v['value'], dest=v['dest'])
...
#     '''
#     helper function to remove a predefined argument
#     '''

...
class SimProc(Process):
    def __init__(self, idx, args, barrier, netdata):
        Process.__init__(self)
        self.idx = idx
        self.args = args
        self.barrier = barrier
        self.netdata = netdata
        self.sim = SumoSim(args.cfg_fp, args.sim_len, args.tsc, args.gui, netdata, args, idx)
    ...
    def run(self):
        ...
        res_total, res_detail = self.sim.sim_stats()


        self.write_result(res_total, self.args.res_fp)
        path = self.args.res_fp.replace('.json', '_det.json')
        self.write_result(res_detail, path)
    ...
    def run_sim(self):
        ...
        start_t = time.time()
        self.sim.gen_sim()        
        # self.sim.create_tscs(default_tsc='websters')
        
        # step 3: run simulation and save output data
        self.sim.run()
    ...
    def write_result(self, data, path):
